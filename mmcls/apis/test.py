    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            for i, (img, img_meta) in enumerate(zip(imgs, img_metas)):
                ...
                img_show = mmcv.imresize(img_show, (600, 600))
                img_show = Pad(img_show)


                if out_dir:
                    if pred_class[i] == img_meta['ori_filename'].split('/')[0]:
                        directory, filename = osp.split(img_meta['ori_filename'])
                        new_filename = 'Success_' + filename
                        new_file_path = osp.join(directory, new_filename)
                    else:
                        directory, filename = osp.split(img_meta['ori_filename'])
                        new_filename = 'Fail_' + filename
                        new_file_path = osp.join(directory, new_filename)             
                    out_file = osp.join(out_dir, new_file_path)
                else:
                    out_file = None
