...
@PIPELINES.register_module()
class Jie_Pad(object):

    def __init__(self, pad_size_h,pad_size_w):
        self.pad_size_h = pad_size_h
        self.pad_size_w = pad_size_w
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            if h > self.pad_size_h or w > self.pad_size_w:
                if h > w:
                    scale = self.pad_size_h / h
                    new_h = int(h * scale)
                    new_w = int(w * scale)
                    img = cv2.resize(img,(new_w,new_h))
                else:
                    scale = self.pad_size_w / w
                    new_h = int(h * scale)
                    new_w = int(w * scale)
                    img = cv2.resize(img,(new_w,new_h))
            else:
                ...
                new_w = w
            # 计算要添加的行和列数量
            pad_h = max(0, self.pad_size_h - new_h)
            pad_w = max(0, self.pad_size_w - new_w)
            h_start = pad_h // 2
            w_start = pad_w // 2
    
            # 创建填充后的图像
            padded_image = np.zeros((self.pad_size_h, self.pad_size_w, 3), dtype=img.dtype)
